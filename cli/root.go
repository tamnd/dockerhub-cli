// Package cli builds the dhub command tree on top of the dockerhub library.
package cli
...
func Root() *cobra.Command {
	app := &App{cfg: dockerhub.DefaultConfig()}

	root := &cobra.Command{
		Use:   "dhub",
		Short: "Browse Docker Hub container images",
		Long: `dhub reads Docker Hub through the public v2 API.
No authentication is required for public images. It returns records as
table, JSON, JSONL, CSV, TSV, or URLs.

dhub is an independent tool and is not affiliated with Docker, Inc.`,
		SilenceUsage:  true,
		SilenceErrors: true,
