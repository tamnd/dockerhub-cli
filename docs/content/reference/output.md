...
describes what users get. Pick a format with `-o`, or let dhub choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
dhub <command> -o table   # aligned columns for reading
dhub <command> -o jsonl   # one JSON object per line, for piping
dhub <command> -o json    # a single JSON array
dhub <command> -o csv     # spreadsheet friendly
dhub <command> -o tsv     # tab-separated
dhub <command> -o url     # just the URL column
dhub <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
dhub <command> --fields id,title,url
```
...
```bash
dhub <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
dhub <command>            # a table, because this is a terminal
dhub <command> | wc -l    # JSONL, because this is a pipe
```
